...
pdf_book_fnaufel <- function(
  ...
  pandoc_args = c(
    ...
    paste0(
      '--filter=',
      system.file(
        "rmarkdown/resources/filters/dollar.py3",
        package = "bd.format"
      )
    ),
    paste0(
      '--filter=',
      system.file(
        "rmarkdown/resources/filters/hl.py3",
        package = "bd.format"
      )
    )
  ),
  ...
  includes = rmarkdown::includes(
    in_header = system.file(
      "rmarkdown/resources/00-preambulo.tex",
      package = "bd.format"
    )
  ),
